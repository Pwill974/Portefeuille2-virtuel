...
def get_secret_password() -> str:
    """Récupère le mot de passe depuis Streamlit Secrets."""
    try:
    ...
    if not password:
        st.error("Le mot de passe n'est pas configuré dans Streamlit Secrets.")
        st.info(
...
def hide_navigation_before_login() -> None:
    st.markdown(
        ...
        .login-card {
            ...
            border-radius: 18px;
            background: linear-gradient(145deg, #101b2f, #0a1526);
...
def require_authentication() -> None:
    ...
    locked_until = float(st.session_state.get("az_locked_until", 0.0))
    ...
    st.markdown(
        ...
        <div class="login-card">
            ...
            <div class="login-subtitle">Portefeuille PEA privé</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if current_time < locked_until:
        remaining = max(int(locked_until - current_time), 1)
        st.error(
            f"Trop de tentatives incorrectes. Réessaie dans {remaining} secondes."
        )
        st.stop()
    ...
    if submitted:
        ...
        if hmac.compare_digest(
            ...
            st.session_state.pop("az_locked_until", None)
            st.rerun()

        failed_attempts = int(
            st.session_state.get("az_failed_attempts", 0)
        ) + 1
        st.session_state["az_failed_attempts"] = failed_attempts

        if failed_attempts >= MAX_ATTEMPTS:
            st.session_state["az_locked_until"] = current_time + LOCK_SECONDS
            st.session_state["az_failed_attempts"] = 0
            st.error(
...
def show_logout_button() -> None:
    if not st.session_state.get("az_authenticated", False):
        return
